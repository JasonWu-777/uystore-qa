...
from config.constants import TIMEOUT_SHORT, BASE_URL, LOGIN_URL, TIMEOUT_LONG,PDF_SAVE_FOLDER
...
def validate_pdf_content(pdf_file_path: str, expected_text: str) -> None:
    ...
    assert expected_text in pdf_text, f"PDF 驗證失敗: 未找到預期文字 '{expected_text}'，文件內容為: {pdf_text}"
...
def get_credentials_MIS(user_credentials_MIS, user_type):
    credentials = user_credentials_MIS[user_type]
    username = credentials["username"]
    password = credentials["password"]
    return username, password
...
def get_conect_URL(page, filename, url):
    ...
    popup_url = f"{BASE_URL}{relative_href}" if relative_href.startswith("/") else relative_href
    return popup_url